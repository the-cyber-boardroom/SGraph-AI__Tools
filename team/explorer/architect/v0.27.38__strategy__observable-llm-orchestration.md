...
We already ship ~80% of the building blocks. Source: `team/explorer/librarian/reality/v0.1.0__what-exists-today__1__libraries.md`.

| Brief component | Existing module(s) | Reuse strategy |
|---|---|---|
| Prompt Inspector | `sg-llm-debug` v0.1.1, `sg-llm-token-viz` v0.1.1, `sg-llm-response-inspector` v0.1.0, `sg-llm-stats` v0.1.1 | Compose into a pre-send "Inspect & Edit" panel. Add one new `llm:before-send` interceptor so the inspector can mutate the request. |
...
| Tool Router | `sg-tool-definition` v0.1.1, `sg-local-bridge` v0.1.0, `sg-tool-runner` v0.1.1 | Add a scoring layer above the existing schema list. Auto-toggle `enabled` based on score. The router itself is a sidecar call. |
...
| Replay Surface | `sg-llm-bundle` v0.1.7 (with `parent_id` fork tree), `sg-llm-bundle-list` v0.1.3, `sg-llm-bundle-viewer` v0.1.0 | Already supports time-travel, fork, import, export. Add "replay with modified params" UI (swap model / tool subset / compression). |
| Vault storage | `vault-client` v1.2.2 + the sgit skill | Each conversation is a vault; each user message is a commit. Bundles already serialise to JSON. |
...
Each conversation is one vault. Structure:

```
{conversation-vault}/
  meta.json                 # title, created_at, default model, sidecar config
  graph/
...
One user message → one commit. Vault read-key sharing = read-only conversation share. Fork = vault clone + new commit thread. Re-open months later = pull vault, hydrate components from `nodes.jsonl` + `turns/`.

This rides on the existing vault primitive — **no vault changes required**. The tool is a vault consumer like every other SG/App.
...
| 3 | One vault per conversation vs many in one vault | **One vault per conversation** for v0.1 — clean sharing/forking model. Revisit if it becomes unwieldy at scale. |
