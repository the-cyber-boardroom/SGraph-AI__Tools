...
**Skill version 3 (2026-09-02).** The scripts print `storyboard generator v3` when they
run and stamp it in the storyboard's footer. If the copy you have prints something
older, or the storyboard it makes has no "How this image was created" list under
each narration and no API cost column in its renders table, you are on a stale
checkout: pull `claude/agent-video-spike-1gpxws-v3` or later before continuing.
...
| `images/`, `images-shorts/`, `clips/`, `frames/` | the reel folder | **no** — regenerated by the scripts; the `.gitignore` in the spike folder blocks them |
| `landscape.webm`, `shorts.webm` | the reel folder | **no** — hand them to the requester with the file-sending tool |
| **The storyboard**: `storyboard.html`, `storyboard.pdf` | the reel folder | **no** — publish `storyboard.inline.html` as an artifact page and send the PDF, named `<Project>__<Title>__Storyboard.pdf` |

"Storyboard" is the name for the document: one page per scene, the narration and how
the image was created on the left, the image on the right, a timecoded transcript at
the end, a renders table with the cost. Videos, stills and the storyboard are
handed over; the repository keeps text only, and a text-only run is under 0.5 MB.
...
node 04-doc.mjs && INLINE=1 node 04-doc.mjs         # storyboard.html + storyboard.pdf, and the single-file copy for publishing
```
...
Publish `storyboard.inline.html` as an artifact page (strip its `<html>/<head>/<body>`
wrapper first; keep `<title>`, `<link>`, `<style>` and `<main>`) and send the videos
and the storyboard PDF with the file-sending tool. None of them is committed.

## What the requester will check first

These are the things that came back as change requests across four rounds; check
them yourself before handing over:

- The opening narration previews the video; it does not repeat scene one.
- No filename bar at the bottom of the frame (`captionBar` off), and in landscape the
  screenshot fills the picture area's full width, cropped around the spotlight.
- The narration is printed in the caption band and the title, date and author are in
  the header of every slide; the closing slide carries the measured numbers and the
  API cost.
- Every spotlight sits on the element it names in both cuts. A rect that is right on
  desktop and wrong on the phone means the target should be an element.
- The storyboard PDF has one scene per page, clips shown as their poster frame, the
  "How this image was created" list under each narration, a timecoded transcript, and
  the cost column. Its footer names the generator version.
