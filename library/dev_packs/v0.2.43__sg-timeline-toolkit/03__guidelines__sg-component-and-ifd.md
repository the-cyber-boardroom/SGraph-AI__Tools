...
**Doc revision:** rev 3 (Pass 1 update — adds §A.6 graveyard of rejected names; mentions sg-config in §M.11; small cross-references for state-restoration-vs-op-event rule)
...
> Read first: **`README.md`** (especially the vocabulary appendix V.1–V.11 and the **Decisions locked at the start of Pass 2** section) and **`01__architecture__sg-toolkit.md`** (especially §3.7 sg-history, §3.8 sg-config, and §5.7 op-based use cases). This doc references both.
...
### A.9 — Names we considered and rejected (the graveyard)

**Why this exists.** Naming debates re-open. Future contributors look at a name and wonder "why this and not the more obvious other?" Without context they pitch the rejected name, the conversation re-runs, and the team spends time re-deriving the same answer. This list ends those re-runs by recording the rejection with rationale.

**For component / module names:**

| Rejected name | Used for | Rejected because |
|---|---|---|
| `sg-clip-strip` | the timeline ruler+lanes component | "clip" is video-specific. "Item" is generic. The component is `<sg-track-strip>`. |
| `sg-timeline` (in toolkit) | the timeline ruler+lanes component | name is taken by the existing v0.1.0 component which stays at its path. New name forces every reference to be re-evaluated, surfacing every coupling. |
| `sg-undoable` | sg-history | the verb shouldn't appear in the name; the name should describe what the thing IS, not what it does to its data. "history" describes the data structure (a record of changes); "undoable" describes a behaviour. |
| `sg-undo` / `sg-redo` | sg-history | same: verbs in names. |
| `sg-state-store` | sg-history or sg-config | too generic; "history" and "config" name actual data structures. |
| `sg-prefs` | sg-config | "preferences" implies user-account-level settings; sg-config is per-tool browser-local configuration, which is a different concern. |
| `sg-settings` | sg-config | same: too implicating of a user-prefs system. |

**For API verb choices:**

| Rejected verb | Used for | Rejected because |
|---|---|---|
| `clone` / `branch` / `push` / `pull` / `merge` | sg-history methods | git-verbs imply git-semantics (remotes, conflict resolution, ancestry). sg-history v0.1.0 is local in-tab state. Using familiar verbs for unfamiliar semantics is worse than using novel verbs honestly. See doc 09 §4. |
| `commit` | sg-history's `record()` | implies a transaction-like boundary; in op-based history the boundary is the op itself. `record(op)` is the honest verb. |
| `checkout` | sg-history's `goTo(position)` | git-verb; same reason. |
| `rollback` | sg-history's `undo()` | implies database-transaction semantics; not what's happening. |

**For tool / sandbox names:**

| Rejected name | Used for | Rejected because |
|---|---|---|
| `sg-toolkit-sandbox` | the sandbox tool | "toolkit" alone is generic; the sandbox is for *this specific* toolkit. |
| `sg-tools-sandbox` | the sandbox tool | confused with the broader `tools/` directory; reads as "a sandbox for the tools repo" rather than "the sandbox for the timeline toolkit." |
| `sg-timeline-toolkit` (alone) | the sandbox tool | the sandbox suffix is required to distinguish the executable demo from the library it demonstrates. The library has no single name; the eight pieces in V.1 ARE the toolkit. |
| `sg-editor-toolkit` | the toolkit collective | "editor" is too narrow; the toolkit also serves Gantt-shaped, log-shaped, animation-shaped tools where "editor" overclaims. |

**For event / op names:**

| Rejected name | Used for | Rejected because |
|---|---|---|
| `clip-moved` | item-moved op | "clip" is video-specific. |
| `timeline-changed` | playhead-changed op | too vague; "playhead-changed" names what changed. |
| `set-name` (op type) | track-renamed op | imperative form ("set X") implies command; declarative past-tense ("X-renamed") describes what happened. Past-tense is the convention. |

**Maintenance:** when a future pack rejects a name in the toolkit space, append the row here. Don't delete entries — the value is in the historical record.
...
### M.11 — State restoration is imperative; user actions are op-shaped events

**Why.** The same component might receive data via two different paths: (1) a user action that changes state, which should emit an op for undo/replay/audit purposes, and (2) a project-load operation that restores state, which should NOT emit ops because the state isn't changing — it's being established. Conflating these means project-load fires hundreds of fake "user added item" ops and the undo stack starts polluted.

**The rule.**
- **Imperative methods restore state.** `setProject(project)`, `setAssets(assets)`, `setTracks(tracks)`, `setItems(items)` — these don't emit ops. They set the component's render-state.
- **User-driven events emit ops.** Drag, click, type, drop — these emit op-shaped events the host routes to sg-history.

**Worked example.** sg-asset-panel:

```js
// Project load — host calls imperatively. NO event fires:
host.querySelector('sg-asset-panel').setAssets([
    {id: 'a-1', name: 'voiceover.mp4', mimeType: 'video/mp4'},
    {id: 'a-2', name: 'background.png', mimeType: 'image/png'},
]);
// Result: panel shows two rows. No 'asset-add-requested' fires.

// User drops a file — component emits op-shaped event:
this.emit('asset-add-requested', {
    op: {
        type: 'asset-add-requested',
        payload: {file, suggestedAssetId: 'a-3'},
        priorState: null,
        sideEffects: ['blob-allocated'],
        reversible: 'with-side-effects',
        timestamp: Date.now(),
        source: 'user-drop',
    },
...
**Anti-pattern.** Calling `setAssets(...)` and having the component re-emit an `asset-add-requested` op for every asset in the array. Project-load now generates op spam and undo can't reach pre-load state. Caught by checklist 04 §B (behaviour preservation in v0.1.55) — saved-project-load MUST NOT generate ops.

### M.12 — `sg-config` field changes flow through the same op pipeline

**Why.** Config changes (`Autosave: false → true`, `Log level: verbose → silent`) are field changes in `<sg-properties-panel>`. They're op-shaped (`reversible: 'pure'`, payload: `{sectionId: 'config', fieldId, fromValue, toValue}`). This means **config changes are undoable** — if the user accidentally toggles a setting they were debugging with, Ctrl+Z restores it.

**The rule.** Hosts wiring config to `<sg-properties-panel>`:
1. Mount the config section using `config.toFields()` to get the V.4 Field shape
2. Listen for `sg-properties-panel:field-changed` events
3. When the section is `'config'`, call `config.set(fieldId, toValue)` — this persists the change AND emits the standard SGCFG event for the rest of the system
4. The same op flows into sg-history (for undo) AND into sg-config's persistence layer

**This means.** `config.set()` is idempotent under op-replay — if sg-history replays a config-change op via `onApply`, the host re-runs `config.set()` which persists again (cheap, no harm).

**Anti-pattern.** Wiring config changes to bypass the op pipeline ("config changes don't need undo"). They do — config IS state, state changes are ops, ops support undo. Don't carve out exceptions; the pipeline is uniform.
...
11. **State restoration is imperative; user actions are op-shaped events** — `setProject(...)` doesn't fire ops; user drops do (M.11)
12. **Config changes flow through the op pipeline** — config is state, state changes are ops, ops are undoable (M.12)
13. **Manifest declares `ops.emits`** — every op type with its category (N.1)
14. **300/350 LOC budget** — split before you cross it (B.3, F.1)
15. **`:host` not `:root`** — shadow DOM scoping (F.2)
16. **Tools register with `SgToolApi`; components don't** (D.1)
17. **Manifest's `actions` MUST list every method** (D.2)
18. **Tools use unique storage keys; toolkit uses defaults only in sandbox** (G.1)
19. **No edits in frozen versions; new minor = new folder** (H.1, H.3)
20. **Branch per session: `claude/{description}-{session-id}`** (H.6)
21. **Explicit `git add` paths; no `git add -A`** (A.1)
22. **One task = one acceptance check = one commit** (K.1)
23. **On ambiguity, ASK** (K.2, A.5)
24. **`DO NOT` is binding** (K.4)
25. **Tick the checklist before claiming done** (A.8, K.1)

End of doc 03 (rev 3). Pass 1 revision complete.
