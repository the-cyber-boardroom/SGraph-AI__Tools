...
`;
    ...
    async _loadTree() {
        ...
        try {
            // Load ref → commit → root tree
            const ref    = await readFileAsJson(this._vault, this._keys.refFileId);
            await decryptEncFields(ref, this._keys.readKey);

            const commitId = ref.head || ref.commit_id;
            if (!commitId) {
                this._showEmpty('Vault is empty. Create the first file!');
                return;
            }

            const commit = await readFileAsJson(this._vault, commitId);
            await decryptEncFields(commit, this._keys.readKey);

            const treeId = commit.tree_id;
            if (!treeId) {
                this._showEmpty('No tree in latest commit.');
                return;
            }

            const tree = await readFileAsJson(this._vault, treeId);
            await decryptEncFields(tree, this._keys.readKey);

            scroll.innerHTML = '';
            this._renderTree(scroll, tree.entries || [], '', 0);
    ...
    _renderTree(container, entries, pathPrefix, depth) {
        for (const entry of entries) {
            const name    = entry.name   || entry.name_enc   || '(encrypted)';
            ...
            if (isDir) {
                ...
                node.addEventListener('click', async (e) => {
                    ...
                    if (expanded && !entry._loaded) {
                        ...
                        try {
                            const subTree = await fetchSubTree(this._vault, entry.tree_id);
                            await decryptEncFields(subTree, this._keys.readKey);
                ...
                node.addEventListener('click', () => {
                    ...
                    this._emit('vault:file-select', {
                        path,
                        fileId: entry.blob_id || entry.content_hash,
                        size:   entry.size ?? 0,
                    });
                });
