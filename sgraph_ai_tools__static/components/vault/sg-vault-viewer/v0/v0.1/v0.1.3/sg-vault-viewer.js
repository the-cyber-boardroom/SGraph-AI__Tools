/**
 * sg-vault-viewer — Vault file viewer and editor panel (session-based).
 *
 * Listens for vault:file-select on [data-vault-bus], uses session.getFile() for
 * cached decryption, and displays the file for viewing/editing. Saves use the
 * mutations module (updateFile) which commits through the session's two-branch
 * model, keeping the tree model in sync.
 *
 * Bus events consumed:
 ...
 *
 * Bus events emitted:
 *   vault:file-saved   — { path, commitId }
...
`;
    ...
    _bindBusEvents() {
        ...
        on('vault:connected',    (e) => {
            this._session = e.detail.session;
        });
        on('vault:disconnected', () => {
            this._session = null;
            this._reset();
        });
    ...
    // ── File loading (cached via session) ──────────────────────────────────
    ...
    _cancelEdit() {
        ...
        this.shadowRoot.getElementById('btn-edit').style.display   = this._path && this._session?.writable ? '' : 'none';
    ...
    async _save() {
        ...
        if (!editor || !this._session || !this._path) return;
        ...
        try {
            ...
            const folderPath = lastSlash > 0 ? '/' + this._path.slice(0, lastSlash) : '/';
            const fileName   = lastSlash >= 0 ? this._path.slice(lastSlash + 1) : this._path;

            const result = await updateFile(this._session, folderPath, fileName, newText);

            this._rawText = newText;
            this._setFooterStatus('Saved.', 'ok');
            this._cancelEdit();
            this._emit('vault:file-saved', { path: this._path, commitId: result.commitId });
