 ...
 *
 * v0.1.1 changes:
 *   - run_code now accepts `language` param ('javascript' | 'python')
 *   - Uses SGSandbox v0.2.0 which supports both JS and Python execution
 *   - BUILTIN_TOOL_DEFS run_code definition updated to document language param
 *   - Python results include { result, stdout, stderr, error } from Pyodide
 *
 ...
 *   run_code     — execute JavaScript or Python code (sg-sandbox v0.2.0)
 *
...
// v0.2.0 sandbox supports both JavaScript and Python
    ...
    },
    {
        ...
        function: {
            ...
            description: [
                ...
                'For JavaScript: use `return` to return a value; `input` variable holds passed-in data.',
                'For Python: `input` is available as a dict; the last expression is the return value;',
                'stdout is captured. Returns { result, stdout, stderr, error } for Python.',
            ].join(' '),
            parameters: {
                type: 'object',
                properties: {
                    ...
                    input: {
                        type: 'object',
                        description: 'Optional data available as the `input` variable inside the code.',
                    },
                },
                required: ['code'],
            },
...
export class SgToolRunner extends HTMLElement {
    ...
    _registerBuiltins() {
        ...
        this.register('run_code', async ({ code, language = 'javascript', input = {} }) => {
            if (!code) throw new Error('code is required');
            if (!this._sandbox) {
                this._sandbox = new SGSandbox({ timeout: 15000 });
            }
            return this._sandbox.execute(code, input, language);
        });
    }
