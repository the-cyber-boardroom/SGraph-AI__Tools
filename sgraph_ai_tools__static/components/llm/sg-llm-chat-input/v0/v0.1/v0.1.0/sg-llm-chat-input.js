 ...
 *
 * Disables itself during in-flight requests (shows Cancel instead of Send).
 *
 * Consumes (on parentElement):
 *   llm:request-start    — disables input, shows Cancel
 *   llm:request-complete — re-enables
 *   llm:request-error    — re-enables
 *   llm:request-cancel   — re-enables
 *
 ...
 *   input.setDisabled(bool)  → programmatic enable/disable
 *
...
export class SgLlmChatInput extends HTMLElement {
    ...
    setDisabled(disabled) {
        ...
        if (cancel) cancel.classList.toggle('visible', disabled && this._requesting);
    }
    ...
    _onPaste(e) {
        // Only intercept if our component is focused or has focus within
        if (!this._shadow.querySelector(':focus-within') && document.activeElement !== this) return;
