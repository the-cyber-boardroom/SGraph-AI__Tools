 ...
 *
 * Changelog:
 *   v0.1.4: Fix resize — :host and .ci-wrap now set width:100% + box-sizing:border-box so
 *     the component reflows correctly when the containing sg-layout panel is resized.
 *   v0.1.3: Expanded file type support — PDF, audio, and video files are now accepted
 *     via the file picker and drag-and-drop. Binary types (PDF/audio/video) are stored
 *     as dataUrl (base64). Type-specific icons: 📄 PDF, 🎵 audio, 🎬 video.
 *     Updated accept string and hint text accordingly.
 *   v0.1.2: _bus() updated to walk [data-llm-bus] ancestor for sg-layout panel compatibility (backwards compatible).
 */
...
export class SgLlmChatInput extends HTMLElement {
    ...
    connectedCallback() {
        this._render();
        this._bindEvents();
        // Reflow textarea when the sg-layout panel is resized
        const ta = this._ta();
        if (ta && typeof ResizeObserver !== 'undefined') {
            this._ro = new ResizeObserver(() => this._resize(ta));
    ...
    _readFile(file) {
        ...
        if (!isImg && !isTxt && !isPdf && !isAudio && !isVideo) return;
        ...
        if (isTxt) {
            ...
            // image, PDF, audio, video — store as base64 dataUrl
            const reader = new FileReader();
            reader.onload = (ev) => {
                this._addAttachment({
                    id, name: file.name,
    ...
    _renderAttachments() {
        ...
        for (const att of this._attachments) {
            ...
            if (att.dataUrl && att.type?.startsWith('image/')) {
                ...
                icon.className   = 'ci-att-icon';
                icon.textContent = _typeIcon(att.type, att.name);
    ...
    _render() {
        this._shadow.innerHTML = `
            ...
            <input type="file" style="display:none" multiple accept="image/*,.pdf,application/pdf,audio/*,video/*,.txt,.md,.js,.ts,.json,.yaml,.yml,.toml,.xml,.csv,.html,.css,.py,.sh,.go,.rs,.java,.c,.cpp,.h">`;
