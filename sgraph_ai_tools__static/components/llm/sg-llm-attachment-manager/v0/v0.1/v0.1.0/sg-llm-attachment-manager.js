...
export class SgLlmAttachmentManager extends HTMLElement {
    ...
    _bindBusEvents() {
        if (!this.hasAttribute('disable-on-send')) return;
        const bus = this._bus();
        const on  = (ev, fn) => {
    ...
    _onPaste(e) {
        // Capture paste when this component is active or nothing else is focused
        const active = document.activeElement;
        const isInput = active && (active.tagName === 'INPUT' || active.tagName === 'TEXTAREA' || active.isContentEditable);
        if (isInput && active !== this && !this.contains(active)) return;
