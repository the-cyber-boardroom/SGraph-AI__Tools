 ...
 *
 * Attributes:
 *   accept         — file picker accept string (default: images + text + PDF + audio + video)
 *   multiple       — allow multiple files (default: true)
 *   disable-on-send — disable input during in-flight requests (default: false)
 *   placeholder    — text shown in empty drop zone (default: "Drop files here or click Browse")
 *
 ...
 *     and stored as dataUrl (base64). Type-specific icons: 📄 PDF, 🎵 audio, 🎬 video.
 *     Updated DEFAULT_ACCEPT and file picker accept string accordingly.
 *   v0.1.2: Three fixes:
...
const DEFAULT_ACCEPT = [
    ...
    'audio/*',
    'video/*',
    '.txt,.md,.js,.ts,.json,.yaml,.yml,.toml,.xml,.csv,.html,.css,.py,.sh,.go,.rs,.java,.c,.cpp,.h',
].join(',');
