...
export class SgLlmBundle extends HTMLElement {
    ...
    loadBundle(id) {
        ...
        if (!bundle) {
            console.warn('[sg-llm-bundle] loadBundle: no data in store for id', id);
            _removeFromIndex(id);
            this._emitLlm(SGL_LLM.BUNDLE_DELETED, { bundleId: id });
            return null;
        }
        console.group('[sg-llm-bundle] loadBundle:', id);
        console.log('config:', bundle.config);
        console.log('chat present:', !!bundle.chat);
        console.log('chat.turns length:', bundle.chat?.turns?.length ?? 'n/a');
        console.log('chat.systemPrompt:', bundle.chat?.systemPrompt || '(empty)');
        if (bundle.chat?.turns?.length) {
            console.log('first turn:', bundle.chat.turns[0]);
            console.log('last turn:', bundle.chat.turns[bundle.chat.turns.length - 1]);
        }
        console.log('full bundle.chat:', bundle.chat);
        console.groupEnd();
    ...
    /**
     * Capture current chat history state from sg-llm-chat-history in the same bus.
     * @returns {{ turns: Array, systemPrompt: string }}
     */
    _captureChat() {
        const bus     = this._bus();
        const history = bus?.querySelector?.('sg-llm-chat-history');
