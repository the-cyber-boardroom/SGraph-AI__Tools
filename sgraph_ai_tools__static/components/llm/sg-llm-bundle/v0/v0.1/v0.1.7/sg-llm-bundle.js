 ...
 *
 * Changelog:
 *   v0.1.7: importBundle(data) — accepts a parsed bundle JSON object (e.g. from a
 *     downloaded .json file), assigns a fresh ID to avoid collisions, saves to
 *     localStorage, emits BUNDLE_SAVED + BUNDLE_LIST_CHANGED, then auto-loads the
 *     imported bundle (emits BUNDLE_LOADED). Returns the imported bundle.
...
export class SgLlmBundle extends HTMLElement {
    ...
    /**
     * Import a bundle from external JSON data (e.g. a downloaded .json file).
     * Assigns a fresh ID to avoid collisions with existing bundles, saves to
     * localStorage, emits BUNDLE_SAVED + BUNDLE_LIST_CHANGED, then loads the
     * bundle (emits BUNDLE_LOADED) so the UI restores immediately.
     *
     * @param {Object} data - Parsed bundle JSON (must be an object)
     * @returns {Object|null} The imported bundle, or null if data is invalid
     */
    importBundle(data) {
        ...
        const bundle = {
            ...data,
            id:        _makeId(),           // fresh ID — no collision with existing
        ...
        // Auto-load so the chat restores immediately after import
        this._activeBundleId = bundle.id;
        this._emitLlm(SGL_LLM.BUNDLE_LOADED, { bundle });
