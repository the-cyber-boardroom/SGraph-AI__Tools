 ...
 *                   + document blocks for anthropic/* models
 ...
 *   - application/pdf with anthropic/* model → document block (OpenRouter
 *     forwards this to Anthropic's API unchanged for Claude models)
            ...
            content: Array.isArray(msg.content)
                ? msg.content.map(part => {
                    if (part.type === 'binary_file') {
                        ...
                        if (part.mime_type === 'application/pdf' && isAnthropicModel) {
                            // OpenRouter routes anthropic/* model calls to Anthropic's API,
                            // which accepts document blocks natively.
                            const base64 = part.data_url?.split(',')[1] ?? '';
