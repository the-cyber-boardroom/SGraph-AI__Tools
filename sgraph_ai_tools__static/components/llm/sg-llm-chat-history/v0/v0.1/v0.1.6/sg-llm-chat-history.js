 ...
 *
 * Changes from v0.1.5:
 *   - "Open ↗" button now converts data: URLs to Blob URLs before calling
 *     window.open(), which browsers block for data: URLs (security policy).
 *
 * Changes from v0.1.3:
...
export class SgLlmChatHistory extends HTMLElement {
    ...
    _onChatMessage(e) {
        const { text = '' } = e.detail ?? {};
        // Merge attachments from event detail (legacy path) with internally tracked ones
        const eventAtts  = e.detail?.attachments ?? [];
        const allAtts    = [...this._attachments, ...eventAtts];
