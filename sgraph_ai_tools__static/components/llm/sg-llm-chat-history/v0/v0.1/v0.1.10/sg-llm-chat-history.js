 ...
 *
 * Changes from v0.1.8:
 *   - Registers itself on the bus element as `__sgLlmChatHistory` when connected,
 *     and deregisters on disconnect. This lets sg-llm-bundle._captureChat() find
 *     the instance even when it lives inside sg-layout's shadow DOM (querySelector
 *     cannot pierce shadow DOM boundaries, causing bundle.chat to always be empty).
 *
 * Changes from v0.1.7:
 *   - Binary file attachments (PDF, audio, video) are now included in the
 *     messages array as `binary_file` content parts. Previously only images and
...
export class SgLlmChatHistory extends HTMLElement {
    ...
    connectedCallback() {
        ...
        this._render();
        this._bindEvents();
        // Register on the bus so sg-llm-bundle can find us across shadow DOM boundaries.
        this._bus().__sgLlmChatHistory = this;
    }
    ...
    disconnectedCallback() {
        ...
        if (bus.__sgLlmChatHistory === this) delete bus.__sgLlmChatHistory;
    }
        ...
        if (role === 'user') {
            ...
            for (const att of attachments) {
                if (att.dataUrl && att.type?.startsWith('image/')) {
                    ...
                    badge.textContent = `${_typeIcon(att.type, att.name)} ${att.name}`;
            ...
            for (const imgObj of images) {
                ...
                if (url) {
                    ...
                    saveBtn.className = 'ch-copy-btn';
                    saveBtn.style.cssText = 'opacity:1;';
                    ...
                    imgActions.appendChild(openBtn);
                    imgActions.appendChild(copyImgBtn);
                    imgActions.appendChild(saveBtn);
 ...
 * User turns with image attachments produce multimodal content arrays using
 * OpenAI-style `image_url` parts. Binary files (PDF, audio, video) are added as
 * `binary_file` parts — sg-llm-request translates these to the provider format.
 * @param {Array} turns
...
function _assembleMsgs(turns, systemPrompt) {
    ...
    for (const t of turns) {
        ...
        const binFiles = (t.attachments ?? []).filter(a => a.dataUrl && !a.type?.startsWith('image/'));
        let text = t.content ?? '';
        for (const att of textAtt) {
            text += `\n\n--- Attachment: ${att.name} ---\n${att.textContent}`;
        }
        const hasParts = images.length > 0 || binFiles.length > 0;
        if (hasParts) {
            ...
            for (const bin of binFiles) {
                parts.push({ type: 'binary_file', mime_type: bin.type, name: bin.name, data_url: bin.dataUrl });
            }
...
/**
 * Draw an image URL onto a canvas and return a PNG Blob.
 * Used for copying images to the clipboard.
 * @param {string} url
 * @returns {Promise<Blob>}
 */
function _imgUrlToPngBlob(url) {
...
/**
 * Return an emoji icon for a given MIME type / filename.
 * @param {string} mimeType
 * @param {string} filename
 * @returns {string}
 */
function _typeIcon(mimeType, filename) {
    if (mimeType?.startsWith('audio/'))           return '🎵';
    if (mimeType?.startsWith('video/'))           return '🎬';
    if (mimeType === 'application/pdf' ||
        filename?.toLowerCase().endsWith('.pdf')) return '📄';
    return '📎';
}
