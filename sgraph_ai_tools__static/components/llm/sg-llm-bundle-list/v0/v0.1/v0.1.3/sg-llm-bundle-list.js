 ...
 *   llm:bundle-inspect-requested   — { bundleId }   user clicks inspect on entry
 *
 * Attributes:
 ...
 *
 * Changelog:
 *   v0.1.3: Export + Import support.
 *     - ↓ download button per entry: downloads the full bundle as
 *       bundle-{id}.json via Blob + createObjectURL.
 *     - Drag-and-drop .json file onto the list → imports via
 *       sg-llm-bundle.importBundle() and auto-loads the bundle.
 *     - "Import" file picker button in the header as an alternative to DnD.
 *     Requires sg-llm-bundle v0.1.7+ for importBundle().
 *   v0.1.2: Added inspect button per entry. Clicking emits
 *     llm:bundle-inspect-requested { bundleId } — consumed by sg-llm-bundle-viewer
 *     to display the raw bundle data for debugging.
...
const BUNDLE_INSPECT_EVENT = 'llm:bundle-inspect-requested';
...
.bl-save-btn:hover,
.bl-import-btn:hover { background: #2d5a8f; }
.bl-header-actions { display: flex; gap: 6px; align-items: center; }
.bl-import-input { display: none; }
.bl-list {
    flex: 1;
    overflow-y: auto;
    padding: 4px 0;
...
.bl-entry:hover .bl-actions { opacity: 1; }
.bl-inspect-btn,
.bl-dl-btn,
.bl-del-btn {
    font-size: 11px;
...
.bl-inspect-btn:hover { color: #7c9ef8; }
.bl-dl-btn:hover      { color: #4ade80; }
.bl-del-btn:hover     { color: #f87171; }
/* Drop zone indicator */
.bl-container.bl-drop-active {
    outline: 2px dashed #7c9ef8;
    outline-offset: -4px;
    background: #0a0a1f;
}
.bl-drop-hint {
...
.bl-container.bl-drop-active .bl-drop-hint { display: block; }
`;
...
export class SgLlmBundleList extends HTMLElement {
    ...
    _render() {
            ...
            <div class="bl-container" part="container">
                <div class="bl-header" part="header">
                    ...
                    <div class="bl-header-actions">
                        ${this.showSaveButton ? '<button class="bl-save-btn" part="save-btn">Save</button>' : ''}
                        <button class="bl-import-btn" title="Import bundle from JSON file">Import</button>
                        <input class="bl-import-input" type="file" accept=".json">
                    </div>
                </div>
                <div class="bl-list" part="list">
                    <div class="bl-drop-hint">Drop bundle JSON to import</div>
                </div>
            </div>`;
        ...
        const importBtn   = this._shadow.querySelector('.bl-import-btn');
        const importInput = this._shadow.querySelector('.bl-import-input');
        importBtn.addEventListener('click', () => importInput.click());
        importInput.addEventListener('change', () => {
            const file = importInput.files?.[0];
            if (file) { this._importFile(file); importInput.value = ''; }
        });
    ...
    _makeEntryEl(entry) {
        ...
        div.innerHTML = `
            ...
            <div class="bl-actions">
                <button class="bl-inspect-btn" title="Inspect bundle data" aria-label="Inspect">{}</button>
                <button class="bl-dl-btn" title="Download bundle as JSON" aria-label="Download">↓</button>
        ...
        div.querySelector('.bl-inspect-btn').addEventListener('click', (ev) => {
            ev.stopPropagation();
            this._emitLlm(BUNDLE_INSPECT_EVENT, { bundleId: entry.id });
        });

        div.querySelector('.bl-dl-btn').addEventListener('click', (ev) => {
            ev.stopPropagation();
            this._downloadBundle(entry.id);
        });
    ...
    /**
     * Download the full bundle JSON for a given id.
     * @param {string} id
     */
    _downloadBundle(id) {
        const bus    = this._bus();
        ...
        const a    = Object.assign(document.createElement('a'), {
            href:     url,
            download: `bundle-${id.slice(-8)}.json`,
        });
        a.click();
    ...
    /**
     * Read a File object, parse as JSON, and import via sg-llm-bundle.
     * @param {File} file
     */
    _importFile(file) {
    ...
    /**
     * Bind drag-and-drop on the container for .json file import.
     */
    _bindDragDrop() {
        const container = this._shadow.querySelector('.bl-container');
        container.addEventListener('dragover', (e) => {
            if ([...e.dataTransfer.types].includes('Files')) {
                e.preventDefault();
                container.classList.add('bl-drop-active');
            }
        });
        container.addEventListener('dragleave', (e) => {
            // Only remove if leaving the container itself, not a child
            if (!container.contains(e.relatedTarget)) {
                container.classList.remove('bl-drop-active');
            }
        });
        container.addEventListener('drop', (e) => {
            ...
            if (file?.name?.endsWith('.json')) this._importFile(file);
        });
    }
