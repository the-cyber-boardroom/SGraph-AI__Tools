...
export function attachDropAffordance(lanesEl, getState, dispatch, pxToTime) {
    ...
    // v0.1.1: window-level safety nets. The dragenter/dragleave `depth`
    // counter only balances when every enter gets a matching leave — shadow-DOM
    // retargeting, drops outside the lanes, or a cancelled drag leave depth
    // stuck at >=1, so the teal .drop-target lane highlight never cleared
    // (it lingered until the next full re-render and re-appeared on every
    // drag). `dragend` fires on the source element when ANY drag finishes —
    // drop, cancel, or Escape — and `drop` anywhere covers drags that end on
    // another element; both bubble to window, where we hard-reset.
    function onWindowDragEnd() { depth = 0; hideAll(lanesEl); }
