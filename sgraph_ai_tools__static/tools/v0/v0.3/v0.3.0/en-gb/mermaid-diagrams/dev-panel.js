/**
 * Mermaid Diagrams — bottom dev panel.
 *
 * Appends a footer bar + collapsible dev panel (Skills / Explorer / Console / Manifest)
 * to the provided layout wrapper. Follows the standard pattern from auth-mvp / send-receive.
 *
 * @module dev-panel
 * @version 0.1.57
 */
...
.skill-md strong{color:#e2e8f0}.skill-md em{color:#cbd5e0}
...
function _buildSkillsPanel() {
    ...
    const SKILLS = [
        ...
        { label: '🤖 Browser / Agent', url: './SKILL-browser.md' },
        { label: '⚡ API Reference',   url: './SKILL-api.md' },
    ];
...
export function buildDevPanel(layoutWrap) {
    ...
    footerInner.innerHTML = `
        ...
        <span style="font-weight:600;font-size:13px;">Mermaid Diagrams</span>
        <span style="color:#64748b;font-size:11px;font-family:'SF Mono',Monaco,monospace;">v0.1.57</span>
        <span style="display:inline-block;padding:1px 6px;border-radius:10px;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.04em;background:rgba(78,205,196,.15);color:#4ecdc4;">live</span>
        <span class="ft-deps" style="color:#64748b;font-size:11px;"></span>
        <span style="margin-left:auto;color:#4ecdc4;font-size:10px;font-weight:600;">Click to open JS API panel</span>
    ...
    fetch('./manifest.json').then(r => r.json()).then(m => {
        ...
        const el = footerInner.querySelector('.ft-deps');
        if (el) el.textContent = `${deps} deps`;
    }).catch(() => {});
}
