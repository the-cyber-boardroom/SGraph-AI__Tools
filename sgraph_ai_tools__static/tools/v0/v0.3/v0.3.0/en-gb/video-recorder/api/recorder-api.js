...
/**
 * Build the shared handoff payload from the last recording.
 * @returns {object}
 * @throws {Error} when no recording exists yet
 */
function _handoffPayload() {
    if (!state.blob) throw new Error('No recording to hand off — record something first.');
    const name = config.recordingName || 'recording';
    return {
        blob:           state.blob,
        audioBlob:      state.blobs?.audio || null,
        suggestedTitle: config.recordingName || `Recording ${new Date().toLocaleDateString('en-GB')}`,
        filename:       `${name.replace(/[^\w-]+/g, '_')}.webm`,
...
/**
 * Open the Video Publisher with the last recording pre-loaded (new tab).
 * Popup blockers may require this to run from a user gesture.
 * @returns {Promise<{ opened: boolean }>}
 */
async function sendToPublisherApi() {
    const { sendToPublisher } = await import('../../video-publisher/handoff/sg-publish-handoff.js');
    return { opened: !!sendToPublisher(_handoffPayload()) };
}

/**
 * Open the YouTube Editor with the last recording pre-loaded (new tab).
 * @returns {Promise<{ opened: boolean }>}
 */
async function sendToYouTubeEditorApi() {
    const { sendToYouTubeEditor } = await import('../../youtube-editor/handoff/sg-youtube-handoff.js');
    return { opened: !!sendToYouTubeEditor(_handoffPayload()) };
}
...
api
    ...
    .register('sendToPublisher',     sendToPublisherApi,     { async: true })
    .register('sendToYouTubeEditor', sendToYouTubeEditorApi, { async: true });
