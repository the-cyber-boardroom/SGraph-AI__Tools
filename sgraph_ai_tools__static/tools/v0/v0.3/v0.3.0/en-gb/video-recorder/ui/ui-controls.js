...
import { initRecIndicator } from './ui-rec-indicator.js';
...
export function initControls(container, state, config, api, emit) {
    ...
    initRecIndicator(container.querySelector('#row-status'));  // ● REC / PREPARING badge (ui-rec-indicator.js)
    ...
    btnRecord.addEventListener('click', async () => {
        ...
        statusEl.textContent   = 'Preparing — don\'t speak yet…';
        _resetPreviewBtn();
        ...
        try {
            ...
            statusEl.textContent = 'Recording — live, speak when ready.';
        } catch (err) {
            btnRecord.disabled   = false;
            btnPreview.disabled  = false;
