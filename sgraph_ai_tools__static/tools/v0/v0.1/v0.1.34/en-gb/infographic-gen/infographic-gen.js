 ...
 *
 * v0.1.34 — Collapsible dev panel via footer bar
 *
 * Layout change: removes the outer sg-layout column split. The main container
 * is now a CSS flex column:
 *   - toolArea    (flex:1)   — inner sg-layout for Create / Results
 *   - devPanel    (0→280px)  — collapsible; tabs: Explorer / Console / Manifest / Skills
 *   - footerBar   (fixed)    — always visible; click to expand/collapse dev panel
 *
 * The footer bar replicates the sg-tool-manifest visual exactly (icon, name,
 * version, status badge, dep count) but its click controls the dev panel rather
 * than showing an inline dep list.
 *
 * The new "Skills" tab fetches and displays the three SKILL-*.md files directly,
 * making them accessible without navigating through Manifest sub-tabs.
 *
 * @version 0.1.34
 */
...
// ── Main layout: flex column (tool area / dev panel / footer bar) ─────────────
...
// ── Collapsible dev panel (starts hidden) ─────────────────────────────────────
...
// Content area (hosts tab panes stacked via position:absolute)
...
for (const t of DEV_TABS) {
    ...
    if (t.tag) {
        ...
        // Skills tab — inline accordion viewer for SKILL-*.md files
...
// Replicates the sg-tool-manifest visual (icon / name / version / status / deps)
// but the click expands the dev panel instead of showing an inline dep list.
...
const api = new SgToolApi({
    ...
    version:  { api: '0.1.0', ui: '0.1.34', content: '0.1.0' },
