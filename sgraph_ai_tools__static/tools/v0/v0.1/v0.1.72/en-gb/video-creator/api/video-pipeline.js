 ...
 *
 * v0.1.72: two config flags, both measured in the agent-makes-the-video spike
 * (team/humans/dinis_cruz/claude-code-web/09/01/v0.2.94__spike-result__agent-makes-the-video/FINDINGS.md):
 *  - paintEveryFrame (default true): canvas.captureStream(fps) only emits a frame when the canvas
 *    is painted, so a slideshow painted once per slide recorded 13 frames in 128 s and every
 *    still was an unrefined P-frame. Repainting at `fps` gives the encoder a real stream.
 *  - captionBar (default true): the filename bar over the bottom 48 px can be turned off when the
 *    slide images carry their own captions.
 *
...
export async function recordSlideshow(state, emit) {
    const { fps, bitrateKbps, width, height, captionBar = true, paintEveryFrame = true } = state.config;
    ...
    try {
        for (let i = 0; i < state.slides.length; i++) {
            ...
            await _drawSlide(recCtx, slide, i, state.slides.length, width, height, captionBar);
            ...
            // Hold the slide for its audio duration; repaint at `fps` meanwhile so captureStream keeps emitting frames.
            await new Promise(r => {
                const timer = setTimeout(() => { clearInterval(repaint); r(); }, duration * 1000);
                const repaint = paintEveryFrame ? setInterval(() => recCtx.drawImage(recCanvas, 0, 0), 1000 / fps) : null;
                if (!paintEveryFrame) clearTimeout(timer), setTimeout(r, duration * 1000);
            });
        }
    } finally {
 ...
 * @param {number} width
 * @param {number} height
 * @param {boolean} [captionBar=true]  Draw the filename bar over the bottom 48 px
 */
async function _drawSlide(ctx, slide, index, total, width, height, captionBar = true) {
