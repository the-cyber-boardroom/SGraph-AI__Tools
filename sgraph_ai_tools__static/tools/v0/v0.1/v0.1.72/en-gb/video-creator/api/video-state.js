 ...
 * @property {boolean} captionBar     Draw the filename bar over the bottom 48 px of every slide (v0.1.72; default true, the pre-v0.1.72 behaviour)
 * @property {boolean} paintEveryFrame Repaint the recording canvas at `fps` while a slide is showing (v0.1.72; default true).
 *                                     canvas.captureStream() only emits a frame when the canvas is painted, so without this a
 *                                     slideshow of N stills is encoded as N frames, each a single P-frame the encoder never refines.
 */
