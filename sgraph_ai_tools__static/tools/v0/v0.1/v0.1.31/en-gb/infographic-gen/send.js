 ...
 *
 * Changes from v0.1.28:
 *   - renderUI:false support — SEND event detail { renderUI: false } skips
 *     all DOM tab creation; the LLM request still runs and window events
 *     still fire. Useful for headless Playwright generation.
 *   - OpenRouter cost API polling — after generation completes, polls
 *     fetchGeneration() every 1s for up to 12 attempts (starting at 2s)
 *     so cost data appears once OpenRouter makes it available (~10s).
 *
 * @version 0.1.31
 */
...
function wireSend(refs) {
    ...
    bus.addEventListener(SGL_LLM.SEND, async e => {
        ...
        // renderUI: false → skip all DOM tab creation (headless mode)
        const renderUI = e.detail?.renderUI !== false;

        // Guard: no API key — flash input and focus it
        if (!apiKey) {
            if (renderUI) {
        ...
        const model_    = model; // alias for closure
        ...
        // ── UI setup (skipped when renderUI:false) ────────────────────────────
        let imgPanel     = null;
        let detailsCtl   = null;
        let panelEl      = null;
        ...
        miniBus.addEventListener(SGL_LLM.REQUEST_COMPLETE, ev => {
            ...
            if (renderUI) {
                ...
                // Poll OpenRouter cost API — available ~10s after completion
                if (genId) {
                    let attemptsLeft = 12;
                    const poll = () => {
                    ...
                    setTimeout(poll, 2000); // first attempt after 2s
                }
            }
            finish();

            window.dispatchEvent(new CustomEvent(SGA_TOOL.GENERATION_COMPLETE, {
                detail: { instanceId, generationId: genId, model: model_, duration: (Date.now() - t0) / 1000, imageSrc: src || null },
            }));
        });
        ...
        miniBus.addEventListener(SGL_LLM.REQUEST_ERROR, ev => {
            ...
            window.dispatchEvent(new CustomEvent(SGA_TOOL.GENERATION_ERROR, {
                detail: { instanceId, generationId: null, model: model_, error: ev.detail?.error || 'Request failed' },
            }));
        });
        ...
        window.dispatchEvent(new CustomEvent(SGA_TOOL.GENERATION_STARTED, {
            detail: { instanceId, generationId: null, model: model_, prompt },
        }));
