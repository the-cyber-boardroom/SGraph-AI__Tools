...
export function mountPropertiesPanel({ host, state, api, getSelectedClipId }) {
    ...
    function onChange(e) {
        ...
        // Skip if a focused text-entry field (inline-rename, textarea,
        // input) lives inside the panel — re-rendering would steal focus
        // mid-edit. The next non-transient change after blur will refresh.
        // Buttons are NOT exempt: in Chrome a click focuses the clicked
        // button, and exempting it made preset-chip / Apply clicks silently
        // skip the re-render (no highlight or Output update until reload).
        const ae = document.activeElement;
        if (ae && root.contains(ae)) {
            const tag = ae.tagName;
            const isTextEntry = tag === 'TEXTAREA' || tag === 'SELECT'
                || (tag === 'INPUT' && ae.type !== 'button' && ae.type !== 'checkbox' && ae.type !== 'color' && ae.type !== 'range')
