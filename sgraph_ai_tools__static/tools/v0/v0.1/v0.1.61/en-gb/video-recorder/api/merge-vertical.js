 ...
 *
 * The compositor is pumped by startBackgroundSafeTicker (a Web Worker timer) and
 * emits frames via requestFrame() on a manual-capture stream, so it keeps drawing at
 * the configured fps even when the recording tab is hidden — which is the normal
 * case during a screen recording. A plain main-thread setInterval would be throttled
 * to <=1Hz by Chrome for hidden tabs (the background-frame-starvation bug).
 *
 ...
 *   getElapsedMs — optional live source of the burned-in footer clock; lets the
 *   caller exclude paused time. Falls back to wall-clock since startedAt.
...
export async function mergeAsShorts(screenStream, cameraStream, options = {}) {
    ...
    // Manual-capture + worker-driven ticker so the composite keeps drawing at ~fps
    // when the recorder tab is hidden (see module header). captureCanvasStream uses
    // captureStream(0)+requestFrame() where supported, else auto-capture at fps.
