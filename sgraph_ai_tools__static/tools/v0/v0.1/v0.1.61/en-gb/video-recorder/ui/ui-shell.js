...
function _initPreviewTab(el, state, config, layout) {
    ...
    igBadge.className = 'preview-live__badge';
    igBadge.textContent = '● INFOGRAPHIC';
    igBadge.style.cssText = 'display:none;background:#3b82f6;';
    igFrame.appendChild(igBadge);
    el.appendChild(igFrame);
    ...
    function _showOnly(which) {
        ...
        igFrame.style.display  = which === 'infographic' ? 'flex' : 'none';
        igBadge.style.display  = which === 'infographic' ? 'block' : 'none';
    }
    ...
    window.addEventListener(SGA_RECORDER.RECORD_START, () => {
        ...
        } else if (mode.includes('screen') && mode.includes('camera') && config.layout === 'infographic') {
            // Infographic layout: 9:16 framed preview from the RAW screen + camera so
            // the user sees how their tall tab fills the frame and can resize it.
            igScreen.srcObject = state.stream;                 // shared screen/tab
            igScreen.play().catch(() => {});
    ...
    window.addEventListener(SGA_RECORDER.RECORD_STOP, () => {
        ...
        igScreen.srcObject  = null;
        igCam.srcObject     = null;
        _showOnly('placeholder');
    });
