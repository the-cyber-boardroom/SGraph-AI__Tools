 ...
 *
 * v0.1.32 changes:
 *   - connect() skip-if-same-key optimisation: if apiKey and model are
 *     unchanged from the current connected state, resolves immediately
 *     without hitting the OpenRouter /models endpoint.
 *   - FIFO → callId: generate() generates a UUID per call, sets it as
 *     callId in the SEND event detail. send.js echoes it in window events.
 *     infographic-gen.js uses a Map<callId, {resolve,reject}> instead of
 *     a FIFO array, so concurrent generate() Promises resolve correctly.
 *
 * @version 0.1.32
 */
...
// Replaces FIFO array — callId-keyed Map allows concurrent generate() calls
// to resolve independently without relying on arrival order.
const _pendingGenerates = new Map(); // callId → { resolve, reject }
...
const api = new SgToolApi({
    ...
    version:  { api: '0.1.0', ui: '0.1.32', content: '0.1.0' },
...
// Skip-if-same-key: if key is unchanged and already connected, resolve immediately.
...
api.register('connect', ({ apiKey, model } = {}) => {
    ...
    // Fast-path: already connected with same key and model
    if (
        newKey   === _connectedKey   &&
        newModel === _connectedModel &&
        _connectedKey !== ''
    ) {
