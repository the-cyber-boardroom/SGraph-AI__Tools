...
export function initApiPane(wrap) {
    ...
    devPanel.className = 'wa-dev-panel';
    ...
    tabBar.className = 'wa-dev-panel__tabs';
    devPanel.appendChild(tabBar);

    const content = document.createElement('div');
    content.className = 'wa-dev-panel__content';
    ...
    for (const t of DEV_TABS) {
        ...
        btn.type = 'button';
        btn.className = 'wa-dev-panel__tab' + (t.id === activeTab ? ' is-active' : '');
        ...
        pane.className = 'wa-dev-panel__pane';
    ...
    footer.className = 'wa-footer-bar';
    const inner = document.createElement('div');
    inner.className = 'wa-footer-bar__inner';
    inner.innerHTML = `
...
function _buildSkills() {
    ...
    wrap.className = 'wa-skills';
    for (const s of SKILLS) {
        const card = document.createElement('div');
        card.className = 'wa-skills__card';
        const hdr = document.createElement('div');
        hdr.className = 'wa-skills__header';
        hdr.innerHTML = `<span>${s.label}</span><span class="wa-skills__chev">▼ expand</span>`;
        const body = document.createElement('pre');
        body.className = 'wa-skills__body';
        body.textContent = 'Loading…';
        hdr.addEventListener('click', () => {
            const open = body.style.display === 'block';
            body.style.display = open ? 'none' : 'block';
            hdr.querySelector('.wa-skills__chev').textContent = open ? '▼ expand' : '▲ collapse';
