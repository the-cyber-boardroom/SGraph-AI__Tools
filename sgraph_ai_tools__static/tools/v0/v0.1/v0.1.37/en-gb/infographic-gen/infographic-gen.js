 ...
 *
 * v0.1.37 changes:
 *   - setSystemPrompt(text) / getSystemPrompt() — set/read the system prompt
 *     textarea directly from the JS API. No more embedding override instructions
 *     inside the user prompt.
 *   - getGenerations(n?) — full request/response audit trail for this session
 *     (ring buffer, last 50). Populated by send.js via onGenerationData callback.
 *   - Skills tab now renders markdown via the new markdown.js module.
 *   - Dev panel extracted to dev-panel.js; orchestrator reduced ~50%.
 *
 * @version 0.1.37
 */
...
placeholder.style.cssText = 'height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;font-family:system-ui,sans-serif;padding:40px;box-sizing:border-box;text-align:center;';
placeholder.innerHTML = `
  ...
  <div style="font-size:13px;color:#4a5568;max-width:420px;line-height:1.8;text-align:left;">
    ...
    Use \u{1F4CE} to attach a reference image in Text mode.<br>
    Use <code style="background:#1a1a3a;padding:1px 5px;border-radius:3px;font-size:12px;">setSystemPrompt()</code> to override the system prompt via the JS API.<br>
    Click the footer bar to open the <strong style="color:#4ECDC4;">JS API panel</strong>.
  </div>
`;
...
// ── Generation log (ring buffer, newest-first) ────────────────────────────────
const GENERATION_LOG_MAX = 50;
const _generationLog = [];
function _onGenerationData(record) {
    _generationLog.unshift(record);
    if (_generationLog.length > GENERATION_LOG_MAX) _generationLog.length = GENERATION_LOG_MAX;
}
...
wireSend({
    ...
    getCurrentMode, getCurrentDoc, getCurrentImage,
    onGenerationData: _onGenerationData,
...
const api = new SgToolApi({
    ...
    version:  { api: '0.1.0', ui: '0.1.37', content: '0.1.0' },
    manifest: './manifest.json',
    skills:   { human: './SKILL-human.md', browser: './SKILL-browser.md', api: './SKILL-api.md' },
});

// connect
let _connectedKey = null, _connectedModel = null;
...
}, { async: true, sanitiseParams: p => ({ ...p, apiKey: p?.apiKey ? '••••' : undefined }), events: [SGA_TOOL.TOOL_CONNECTED] });
...
api.register('generate', ({ prompt, model, renderUI = true } = {}) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        bus.dispatchEvent(new CustomEvent(SGL_LLM.SEND, { detail: { renderUI, callId }, bubbles: false }));
    });
}, { async: true, events: [SGA_TOOL.GENERATION_STARTED, SGA_TOOL.GENERATION_COMPLETE, SGA_TOOL.GENERATION_ERROR, SGA_TOOL.GENERATION_CANCELLED] });
...
// getGenerations
api.register('getGenerations', (n) =>
    typeof n === 'number' ? _generationLog.slice(0, n) : [..._generationLog],
    { async: false }
);

// system prompt
api.register('setSystemPrompt', text => { advTextarea.value = String(text); }, { async: false });
api.register('getSystemPrompt', ()   => advTextarea.value,                    { async: false });

// prompt / model / template / stop
...
api.register('setTemplate', name => {
    const tmpl = TEMPLATES.find(t => t.id === name || t.label.toLowerCase() === String(name).toLowerCase());
