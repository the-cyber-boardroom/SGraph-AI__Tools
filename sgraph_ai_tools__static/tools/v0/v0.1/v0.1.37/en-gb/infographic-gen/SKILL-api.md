...
**Version:** ui=0.1.37, api=0.1.0, content=0.1.0
...
side-effects:
  ...
  - records a full request/response entry in the generation log (accessible via getGenerations())
errors:
...
Returns the full request/response audit trail for this session.

```
signature:   getGenerations(n?) → Array<record>
async:       false
params:
  n  number  optional  Max records to return, newest-first.
                       Omit for all (ring buffer: last 50).
returns: Array of generation records, newest first. Each record:
  callId        string        UUID matching the generate() call and window events
  generationId  string|null   OpenRouter generation ID
  timestamp     number        Date.now() at request dispatch
  model         string        Model used
  prompt        string        Text prompt sent
  request       object        Full request detail:
    messages    array         Exact multimodal message array sent to sg-llm-request.
                              Each message: { role, content } where content is a string
                              or an array of content parts (text + image_url).
    model       string        Model ID sent in the request
  response      object|null   null on error/cancel. On success:
    content           string        Full text/SVG content from the model
    promptTokens      number        Input token count
    completionTokens  number        Output token count
    latencyMs         number        Network + model latency in milliseconds
    finishReason      string        'stop' | 'length' | 'content_filter' | etc.
    images            array|null    Image URLs extracted from response (if any)
    rawChunks         array|null    Raw SSE chunks (streaming mode only)
    rawResponse       object|null   Full response object (non-streaming mode only)
  imageSrc      string|null   Rendered image URL (data: or https:), null if SVG/error
  duration      number        Wall-clock seconds from send to outcome
  status        string        'complete' | 'error' | 'cancelled'
  error         string|null   Error message if status='error', else null

note:
  request.messages is the exact array passed to sg-llm-request — including any
  image_url parts if a reference image was attached. This gives full visibility
  into multimodal request construction for debugging or audit purposes.
```

### setSystemPrompt / getSystemPrompt

Set or read the system prompt textarea directly from the JS API.

```
setSystemPrompt(text: string) → void
  Sets the system prompt textarea (advTextarea.value = String(text)).
  Takes effect on the next generate() call.
  Pass '' (empty string) to clear the override — the next send will fall back
  to the mode default (text mode default or document mode default).

getSystemPrompt() → string
  Returns the current value of the system prompt textarea.
  May be the mode default or an override set via setSystemPrompt().
```

**Motivation:** Without this method, controlling visual style, colour scheme, or
rendering constraints required embedding override instructions *inside* the user
prompt (e.g. `"CRITICAL INSTRUCTION: Override all previous design instructions..."`).
`setSystemPrompt()` provides a clean separation — the system prompt is set once and
all subsequent `generate()` calls use it automatically.

```js
// Set a custom system prompt, then generate
window.__tool.setSystemPrompt(
    'You are a professional infographic designer. Use only dark backgrounds, ' +
    'teal and white text, and a 16:9 aspect ratio. Always output SVG.'
);
await window.__tool.generate({ prompt: 'A timeline of AI milestones' });
...
`getGenerations()` records the exact messages array (including any image_url parts) in
`request.messages` for audit/debug purposes.
