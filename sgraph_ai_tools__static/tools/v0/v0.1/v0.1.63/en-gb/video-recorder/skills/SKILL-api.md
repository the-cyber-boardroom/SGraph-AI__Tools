...
| `setRecordingName` | `{ name: string }` | `{ name }` | no |
...
type RecordingMode =
  ...
  'viz+audio' | 'screen+viz+audio';   // viz modes require an audio source
...
interface RecordingConfig {
  ...
  recordingMode:      'combined' | 'combined+separate' | 'separate';  // how many output files
  layout:             'landscape' | 'shorts';     // camera+screen composite only
  vizMode:            string;                      // sg-audio-viz style for viz modes
  recordingName:      string;                      // download filename + tab title
}
...
| `tool:record:armed` | `{ mode }` | `startRecording` began — acquiring media / camera warm-up; NOT live yet (don't speak) |
...
window.__tool.meta.getVersion()  // { api: '0.1.0', ui: '0.1.63', content: '0.1.0' }
window.__tool.meta.getEvents()   // ['tool:ready','tool:preview:start',…]
window.__tool.meta.health()      // { status: 'ready', methodCount: 15, … }
