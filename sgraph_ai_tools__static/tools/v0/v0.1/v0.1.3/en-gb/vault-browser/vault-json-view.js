...
export class VaultJsonView extends HTMLElement {
    ...
    /** Convert a ref value like "ref-abc123" to the full vault path "bare/refs/ref-abc123" */
    _refToFileId(val) {
        ...
        if (val.startsWith('branch-named-'))   return `bare/indexes/${val}`
        if (val.startsWith('branch-clone-'))   return `bare/indexes/${val}`
        return `bare/data/${val}`
    }

    _looksLikeRef(val) {
        return /^(obj-|ref-|key-|idx-|branch-|bare\/)/.test(val)
    }
