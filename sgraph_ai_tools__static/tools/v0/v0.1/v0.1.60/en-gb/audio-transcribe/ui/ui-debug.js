...
export function mountDebug({ root, api }) {
    ...
    function card(x) {
        ...
        const body = x.status === 'error'
            ? `<span class="at-muted">⚠ ${esc(x.error || 'failed')}</span>`
            : esc(r.content || '');
        ...
        return `
            <div class="at-xch at-xch--${esc(x.status)}">
                <div class="at-xch__head">
                    ...
                    <span class="at-chip at-chip--${esc(x.status === 'done' ? 'done' : 'error')}">${esc(x.status)}</span>
    ...
    function onExchange(e) { if (e && e.detail) { log.unshift(e.detail); if (log.length > 100) log.length = 100; render(); } }
