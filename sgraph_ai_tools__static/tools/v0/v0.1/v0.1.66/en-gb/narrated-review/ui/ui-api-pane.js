 ...
 * + footer bar — the standard pattern (copied from video-publisher).
 * @module ui-api-pane
 */
...
export function initApiPane(wrap) {
    ...
    devPanel.className = 'nr-dev-panel';
    ...
    tabBar.className = 'nr-dev-panel__tabs';
    devPanel.appendChild(tabBar);

    const content = document.createElement('div');
    content.className = 'nr-dev-panel__content';
    ...
    for (const t of DEV_TABS) {
        ...
        btn.type = 'button';
        btn.className = 'nr-dev-panel__tab' + (t.id === activeTab ? ' is-active' : '');
        ...
        pane.className = 'nr-dev-panel__pane';
    ...
    footer.className = 'nr-footer-bar';
    const inner = document.createElement('div');
    inner.className = 'nr-footer-bar__inner';
    inner.innerHTML = `
...
function _buildSkills() {
    ...
    wrap.className = 'nr-skills';
    for (const s of SKILLS) {
        const card = document.createElement('div');
        card.className = 'nr-skills__card';
        const hdr = document.createElement('div');
        hdr.className = 'nr-skills__header';
        hdr.innerHTML = `<span>${s.label}</span><span class="nr-skills__chev">▼ expand</span>`;
        const body = document.createElement('pre');
        body.className = 'nr-skills__body';
        body.textContent = 'Loading…';
        hdr.addEventListener('click', () => {
            const open = body.style.display === 'block';
            body.style.display = open ? 'none' : 'block';
            hdr.querySelector('.nr-skills__chev').textContent = open ? '▼ expand' : '▲ collapse';
