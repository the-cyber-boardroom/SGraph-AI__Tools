 ...
 *   tool:generation:complete  — when REQUEST_COMPLETE fires (includes imageSrc)
 *   tool:generation:error     — when REQUEST_ERROR fires
 *   tool:generation:cancelled — when REQUEST_CANCEL fires
 *
 * Changes from v0.1.26:
 *   - No-key guard now flashes apiKeyEl red + focuses it (was silent return)
 *   - GENERATION_COMPLETE detail now includes imageSrc: string|null
 *
 * @version 0.1.28
 */
...
function wireSend(refs) {
    ...
    bus.addEventListener(SGL_LLM.SEND, async e => {
        ...
        miniBus.addEventListener(SGL_LLM.REQUEST_COMPLETE, ev => {
            ...
            // ── Phase 0: window event (includes imageSrc for API consumers) ──────
            window.dispatchEvent(new CustomEvent(SGA_TOOL.GENERATION_COMPLETE, {
                detail: { instanceId, generationId: genId, model, duration: (Date.now() - t0) / 1000, imageSrc: src || null },
            }));
        });
