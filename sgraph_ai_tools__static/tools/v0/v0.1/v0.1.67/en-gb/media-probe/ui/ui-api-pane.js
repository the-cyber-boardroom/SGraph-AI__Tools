...
export function initApiPane(wrap) {
    ...
    devPanel.className = 'mp-dev-panel';
    ...
    tabBar.className = 'mp-dev-panel__tabs';
    devPanel.appendChild(tabBar);

    const content = document.createElement('div');
    content.className = 'mp-dev-panel__content';
    ...
    for (const t of DEV_TABS) {
        ...
        btn.type = 'button';
        btn.className = 'mp-dev-panel__tab' + (t.id === activeTab ? ' is-active' : '');
        ...
        pane.className = 'mp-dev-panel__pane';
    ...
    inner.className = 'mp-footer-bar__inner';
    inner.innerHTML = `
...
function _buildSkills() {
    ...
    wrap.className = 'mp-skills';
    for (const s of SKILLS) {
        const card = document.createElement('div');
        card.className = 'mp-skills__card';
        const hdr = document.createElement('div');
        hdr.className = 'mp-skills__header';
        hdr.innerHTML = `<span>${s.label}</span><span class="mp-skills__chev">▼ expand</span>`;
        const body = document.createElement('pre');
        body.className = 'mp-skills__body';
        body.textContent = 'Loading…';
        hdr.addEventListener('click', () => {
            const open = body.style.display === 'block';
            body.style.display = open ? 'none' : 'block';
            hdr.querySelector('.mp-skills__chev').textContent = open ? '▼ expand' : '▲ collapse';
