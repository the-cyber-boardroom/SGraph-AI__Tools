...
export function detectSchema(data) {
    ...
    if (data.tree_id && (data.message !== undefined || data.message_enc !== undefined) && (data.parents || data.parent))  return 'commit_v1'
    if (data.entries && Array.isArray(data.entries) && (data.entries[0]?.blob_id !== undefined || data.entries[0]?.tree_id !== undefined)) return 'tree_v1'
