/**
 * PlaybookLM — UI shell builder (v0.1.43).
 *
 * Wires the state-aware stepper: a getCompletedSteps() callback derives
 * which steps have pipeline data so ✓ indicators survive back-navigation.
 * State changes refresh the stepper automatically.
 *
 * @module ui-shell
 * @version 0.1.4
 */
...
export async function buildShell({ state, manifest }) {
  ...
  // back-navigation (clicking step 1 no longer clears steps 2-4).
  const getCompletedSteps = () => {
    ...
    if (state.getSlideResults().some(r => r.status === 'complete')) done.add(4);
    return done;
  };
