 ...
 * Cost is recorded on every successful call via state.recordCost().
 *
...
/**
 * Approximate USD cost per 1M tokens (input / output) for known OpenRouter models.
 * Values are estimates and may drift; the table is best-effort.
 *
...
/**
 * Flat USD cost estimate per image for known image models.
 * Used when the API response lacks token-based usage.
 *
...
/**
 * Calculate USD cost from token usage for a given model.
 *
 * @param {string} model
...
/**
 * Calculate USD cost for an image call (flat estimate per image).
 *
 * @param {string} model
 * @param {object} [rawResponse] - Full OpenRouter response (may contain usage)
 * @returns {number} USD cost
 */
function _calcImageCost(model, rawResponse) {
  // Some image models return token usage; try that first
  const usage = rawResponse?.usage;
  if (usage?.prompt_tokens || usage?.completion_tokens) {
    return _calcTextCost(model, usage);
  }
  // Fall back to flat per-image estimate
 ...
 * Records cost via state.recordCost() on success.
 *
...
export async function _callTextLLM(state, userPrompt, systemPrompt) {
  ...
  try {
    ...
    // Record cost if usage data is present
    if (data.usage) {
      state.recordCost(_calcTextCost(state.textModel, data.usage));
    }
...
export function _callImageLLM(state, systemPrompt, userPrompt) {
  ...
  return new Promise((resolve, reject) => {
    ...
    miniBus.addEventListener(SGL_LLM.REQUEST_COMPLETE, ev => {
      ...
      if (src) {
        state.recordCost(_calcImageCost(imageModel, ev.detail?.rawResponse));
        resolve(src);
      } else {
