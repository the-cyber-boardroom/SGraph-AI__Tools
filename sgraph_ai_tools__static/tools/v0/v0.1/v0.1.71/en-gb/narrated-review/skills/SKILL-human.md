...
## Capturing what you DID, not just what you showed (v0.1.7)

A screenshot cannot show the path your mouse took to find a control, the pause
before you clicked, the shortcut you reached for instead of the menu, or the
console error that appeared while nobody was looking at the console. For a UX
question those *are* the answer.

**This needs the SG Page Recorder extension**, and there is no way around that. A
screen-capture stream hands the browser pixels and audio and nothing else — that
is the security boundary, not a gap in this tool. (Measured, not assumed: the
only display properties Chrome lets you constrain are `displaySurface` and
`restrictOwnAudio`.) So the checkboxes are shown either way, and when the
extension is missing they say so rather than sitting there greyed out.

**To use it:** install the extension, open the page you want to narrate, click
the extension icon **on that tab**, choose what to record, press Start — then
press **Attach recorded tab** in narrated-review. You arm the tab, not the tool:
a web page should never be able to start recording another page.

| Feed | What lands in `input.json` |
|---|---|
| Mouse | the path (decimated for reading), clicks, and **which element** was hit |
| Keyboard | *off by default* — see below |
| Console | logs, warnings, uncaught errors, unhandled rejections |
| Network | method, path, status, duration — **never headers or bodies** |
| Probes | your own JavaScript, run on a trigger, result on the timeline |

**Keyboard has three settings, and the middle one is the useful one.** `off` is
the default. **`which keys`** records that you pressed a key and which key it was,
but replaces printable characters with `·` — you get shortcuts, navigation,
rhythm and hesitation without the recording containing what you typed. `literal
text` records the characters and has to be chosen deliberately.

Two rules hold whatever you choose:

- **Password and payment fields give up nothing** — not the characters, not the
  length, only that typing happened.
- **A shortcut is not typed text.** `Ctrl-S` is recorded as `Ctrl-S` even in the
  masked mode: hiding it protects nothing and loses the most useful signal there
  is.

Every refusal is counted, so a reader can tell "typed nothing" from "we refused
to record it".

`input.json` goes into both zips, with a `moments[]` array that has already
sliced everything into the capture it belongs to.
