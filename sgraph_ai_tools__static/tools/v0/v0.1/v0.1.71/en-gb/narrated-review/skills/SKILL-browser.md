...
## The input feed in tests (v0.1.7)

The extension cannot be armed from Playwright — `activeTab` needs a real click on
the extension icon — so drive `nr-input.js` directly instead. That is where the
...
await page.evaluate(async () => {
  ...
  st.state.startedAt = 1000;                 // the session clock origin
  inp.ingest({ tabId: 7, events: [
    { t: 900,  k: 'move', x: 1, y: 1 },      // BEFORE the session — dropped, counted as `outside`
    { t: 1700, k: 'click', x: 20, y: 20, el: { sel: 'button#go' } },
  ]});
  return inp.inputToJson();
});
```

The extension's own recorder is testable by loading `page-hooks.js` then
`content-input.js` into a page with a stubbed `chrome.runtime` — see
`tests/playwright/sg-page-recorder-smoke.js`. Order matters: MAIN world first,
same as `service-worker.js` does it.

**DOM contract (added):** `#nr-input` · `#nr-input-state` · `#nr-input-why` ·
`#nr-in-mouse` · `#nr-in-keys` · `#nr-in-console` · `#nr-in-net` ·
`#nr-in-attach` · `#nr-in-detach`

**Events:** `nr:input:started`, `nr:input:batch`, `nr:input:stopped`.
