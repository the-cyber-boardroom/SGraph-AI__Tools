...
## The input feed (v0.1.7) — mouse, keyboard, console, network, probes

`inputAvailability()` · `listRecordableTabs()` · `attachInput({tabId})` ·
`detachInput()` · `getInput()` · `getInputTrack()` · `runPageProbe({js, on})`

### Why none of this comes from the capture stream

**Measured, not assumed.** In current Chrome the only constrainable display
properties are `displaySurface` and `restrictOwnAudio` — `cursor` is not even in
`getSupportedConstraints()` any more. `CaptureController`, the newest
surface-control API, exposes `forwardWheel`, `setFocusBehavior` and the zoom
methods. Every one of those points **outward**, into the captured tab. There is
no inbound channel by design: screen-sharing a bank tab must not let the sharer
read the keyboard.

So mouse, keys, console and network cannot be recovered from a capture stream at
any quality, and the SG Page Recorder extension is the only honest route.
`inputAvailability()` returns that reasoning as `reason` so a UI can state it
instead of greying a control out.

### The tool cannot arm a tab, deliberately

`chrome.scripting.executeScript` runs under `activeTab`, granted only in response
to a click on the extension's own icon. `attachInput` picks up a tab that is
**already** armed, and when it cannot find one it says exactly what to do rather
than failing vaguely. A page starting a recording on another page is precisely
the capability that should not exist.

### Alignment, and the clock to be careful about

Events arrive stamped `Date.now()` from another tab and are converted once, on
arrival, by subtracting `state.startedAt`. That is **wall-clock** alignment, not
the audio clock the PCM slices use; the two drift by tens of milliseconds over a
long session. `schema.clock` says `'wall'` so nobody has to guess.

An event from **before** `startedAt` is dropped and counted as `outsideSession`.
The first implementation clamped it with `Math.max(0, …)`, which placed it inside
capture 1 — a moment at which it did not happen. Inventing a position is worse
than having none.

### `input.json`

`events[]` is everything in order. `moments[]` is the same data already sliced
into the capture whose bounds contain it, with per-capture counts, the clicks
(each naming the element hit), the keys, console errors, failed requests, probe
results, and the mouse path **decimated** by perpendicular distance — a raw 30 Hz
path is thousands of points that mostly say "still moving in a straight line".
Prefer `moments[]` unless you are literally replaying the session.

### Privacy is in the data model, not the docs

- Password fields, `autocomplete="cc-*"`, `one-time-code`, and anything inside
  `[data-sg-no-capture]`: **nothing is recorded but the fact of typing**, in every
  mode.
- Default keyboard mode masks printable characters to `·` — the key is kept, the
  character is not.
- **A modifier makes it a shortcut, not content.** `Ctrl-S` is recorded literally
  even when masked: nobody types a password with Ctrl held, and the shortcut is
  the signal a UX question is about. (A test caught the first version masking it.)
- Network: **no headers, no bodies, at any setting.** Query strings stripped
  unless `fullUrls` is set — ids and tokens hide there.
- Every refusal is counted in `redacted`, because an absent event and a withheld
  one must not look the same.

### Scripted probes

`runPageProbe({ js, on })` runs operator-authored JavaScript in the page, with the
result recorded as a `probe` event in timeline order. Triggers: `manual`, `start`,
`stop`, `click`, `keydown`, `scroll`, `interval`. It captures what a screenshot
cannot — a computed value, a store's state, a filtered list length. It is
arbitrary code execution in the page: `new Function` rather than `eval` so the
snippet cannot reach the recorder's closure, results pass through the same
size-bounded serialiser as console arguments, a probe that throws records the
error instead of taking the recorder down, and every probe and result is logged
to `actions.json`.
