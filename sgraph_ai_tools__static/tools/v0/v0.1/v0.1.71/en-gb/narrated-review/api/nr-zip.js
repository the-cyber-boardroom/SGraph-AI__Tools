...
export function buildReadme(include = {}) {
    ...
    return [
        ...
        '| `input.json` | What the operator DID on the recorded page — mouse path, clicks, keys, console and network — on the same clock as the captures. Present only if the page recorder was attached. |',
...
export function buildSessionEntries(include = {}) {
    ...
    // Only when something was actually recorded — an empty input.json in every
    // bundle would teach readers to ignore the file that matters when it is full.
    if (inputSummary().total) entries.push({ path: 'input.json', text: JSON.stringify(inputToJson(), null, 2) });
