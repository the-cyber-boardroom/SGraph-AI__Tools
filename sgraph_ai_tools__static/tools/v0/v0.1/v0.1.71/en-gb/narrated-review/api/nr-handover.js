...
import { inputToJson, inputSummary } from './nr-input.js';
...
export function buildHandoverReadme() {
    ...
    return [
        ...
        ...(inputSummary().total ? [
            '5. **`input.json`** — what the operator DID on the page: the mouse path taken to reach a',
            '   control, clicks with the element hit, keystrokes, console errors and network calls,',
            '   all on the same clock as the captures. `moments[]` has it pre-sliced per capture.',
            '   **This is the only file that can answer a UX question** — a screenshot cannot show',
            '   hesitation, a path, or an error nobody was looking at. Keystrokes in password and',
            '   payment fields are never recorded; `redacted` counts what was refused.',
        ] : []),
        '',
        ...
        ...(inputSummary().total
            ? ['| `input.json` | Mouse, keys, console and network from the recorded page. |'] : []),
...
export function buildHandoverEntries() {
    ...
    if (inputSummary().total) {
        entries.push({ path: 'input.json', text: JSON.stringify(inputToJson(), null, 2) });
    }
