/* =================================================================================
   ...
   v0.1.70 changes (2026-08-26):
     - No slug changes. 'narrated-review' v0.1.6 — durability, history and a
       bundle shaped for agents, all from real use:
       * AUTOSAVE, on by default and always visible. Every mutation checkpoints,
         logs and schedules a write; a localStorage beacon lets the next boot
         offer back a session that was in progress. beforeunload guards unsaved
         changes and any live recording. The audio take is the one thing not
         written mid-recording — stated in the UI rather than hidden.
       * UNDO/REDO by snapshot (blobs by reference), plus a separate append-only
         action log that is never rewound and ships as actions.json.
       * STREAMING CLEANUP, default on. Cleanup only ever looks backwards, so
         running it during the recording is the identical computation started
         earlier — the wait after Finish drops from one model call per capture
         to roughly one in total. A parallel option exists and names its real
         quality cost.
       * AGENT HANDOVER ZIP: no audio, no PDF, plus uncertain.json (every
         flagged span with its context) and actions.json. uncertain.json is in
         the full export too — it is the most-used part of the bundle.
   v0.1.71 changes (2026-08-26):
     - No slug changes. 'narrated-review' v0.1.7 adds a FIFTH FEED: what the
       operator did, beside what they said and showed — mouse path, clicks,
       keystrokes, console, network, and operator-authored JS probes, all on
       the capture clock, shipped as input.json in both bundles.
       It needs the new SG Page Recorder extension
       (extension/sg-page-recorder/v0.1.0/) and CANNOT work without one: a
       screen-capture stream carries pixels and audio only. Measured, not
       assumed — the only constrainable display properties Chrome reports are
       displaySurface and restrictOwnAudio, and CaptureController's methods
       (forwardWheel, setFocusBehavior, zoom) all point OUTWARD into the
       captured tab. There is no inbound channel by design.
       Keyboard is off by default and masks printable characters; password and
       payment fields record nothing in any mode; network is metadata only,
       never headers or bodies. Every refusal is counted, because an absent
       event and a withheld one must not look the same.
   ================================================================================= */
