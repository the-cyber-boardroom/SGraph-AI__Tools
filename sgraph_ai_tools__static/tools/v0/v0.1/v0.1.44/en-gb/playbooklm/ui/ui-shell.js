/**
 * PlaybookLM — UI shell builder (v0.1.44).
 *
 * Changes from v0.1.43:
 *  - getCompletedSteps: step 4 is now accessible whenever briefs exist
 *    (not only when slides are complete). Fixes "step 4 disappears when
 *    navigating back to step 3 without having generated slides".
 *  - Right panel: second tab "Monitor" added with sg-llm-response-inspector,
 *    sg-tool-api-console, and a Gen IDs list backed by plm:llm-log events.
 *
 * @module ui-shell
 * @version 0.1.5
 */
...
/**
 * Build the full UI shell: layout, stepper, step panels, dev panel, monitor.
 *
 * @param {object} opts
...
export async function buildShell({ state, manifest }) {
  ...
  // ── State-aware stepper ───────────────────────────────────────────────────
  // Derives which steps have data from PipelineState so ✓ marks survive
  // back-navigation.
  //
  // Step 4 is accessible whenever briefs exist — even without generated slides.
  // This lets the user navigate back to Step 3 and return to Step 4 without
  // losing access to the generate panel. Step 5 requires at least one completed
  // slide (there must be something to export).
  const getCompletedSteps = () => {
    ...
    if (state.getSlideBriefs().length > 0)                         done.add(4); // accessible when briefs ready
  ...
  // Keep step indicators in sync with pipeline data
...
/**
 * Wire the connection bar. Restores API key from sessionStorage on load.
 * Saves on successful connect. Injects cost display.
 *
