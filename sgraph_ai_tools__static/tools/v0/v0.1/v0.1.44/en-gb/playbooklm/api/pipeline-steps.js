 ...
 *
 * v0.1.44 changes:
 *  - MODEL_PRICING: added 'anthropic/claude-haiku-4.5' (OpenRouter short form).
 *  - _callTextLLM 'done' log: added rawResponse, generationId (data.id), latencyMs.
 *  - _callImageLLM 'response-raw' log: added rawDetail (actual object, not just JSON)
 *    and generationId for the Gen IDs monitor tab.
 *
 * v0.1.43 changes:
 ...
 * The dev panel LLM Log tab and the Monitor panel both listen for these.
 *
 * @param {object} detail
 * @returns {void}
 */
 ...
 * Logs full request/response to the dev panel LLM Log tab and Monitor panel.
 *
...
export async function _callTextLLM(state, userPrompt, systemPrompt) {
  ...
  try {
    ...
    _logLLM({
      ...
      usage:           data.usage,
      cost,
      generationId:    data.id,          // OpenRouter generation ID for cost lookup
      rawResponse:     data,             // full response for monitor panel relay
 ...
 * Logs full request/response detail to the dev panel LLM Log tab and Monitor panel.
 *
 * @param {import('./pipeline-state.js').PipelineState} state
 * @param {string} systemPrompt
 * @param {string} userPrompt
...
export function _callImageLLM(state, systemPrompt, userPrompt) {
  ...
  return new Promise((resolve, reject) => {
    ...
    miniBus.addEventListener(SGL_LLM.REQUEST_COMPLETE, ev => {
      ...
      // Log full detail: rawDetail (object) for monitor relay, detailJson for dev panel display
      _logLLM({
        id,
        type:          'image',
        phase:         'response-raw',
        model:         imageModel,
        ts:            Date.now(),
        generationId:  ev.detail?.rawResponse?.id,  // OpenRouter generation ID
        rawDetail:     ev.detail,                   // actual object for monitor relay
        detailJson:    _safeJson(ev.detail),         // stringified for dev panel display
      });
...
export async function generateSlide(state, params) {
  ...
  try {
    const userPrompt = `Title: ${brief.title}\n\nVisual brief: ${brief.prompt}`;
    const imageSrc = await _callImageLLM(state, DEFAULT_INFOGRAPHIC_SYSTEM_PROMPT, userPrompt);
    state.setSlideResult(index, { status: 'complete', imageSrc });
...
async function _exportZIP(results, briefs) {
  ...
  a.href = url;
  a.download = 'playbooklm-deck.zip';
  a.click();
