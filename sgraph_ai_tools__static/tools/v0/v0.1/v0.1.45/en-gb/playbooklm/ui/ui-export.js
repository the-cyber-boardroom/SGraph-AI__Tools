 ...
 *
 * v0.1.1 changes:
 *  - Session Archive export: full ZIP containing slides (selected versions),
 *    all slide versions, presentation strategy doc, slide briefs, per-slide
 *    prompt history, cost summary, and original sources.
 *  - SG/Send share button: builds the archive ZIP then opens send.sgraph.ai
 *    in a new tab. A toast instructs the user to upload the downloaded file.
 *
 * @module ui-export
 * @version 0.1.1
 */
...
export function buildExportPanel(container, state, setStep) {
  container.style.cssText =
    'overflow-y:auto;height:100%;box-sizing:border-box;padding:16px;background:#0d0d1a;font-family:system-ui,sans-serif;display:flex;flex-direction:column;gap:16px;';
  ...
  container.innerHTML = `
    ...
    <div class="plm-export-card">
      <div class="plm-export-card-header">
        ...
        <div>
          ...
          <div class="plm-export-desc">Landscape 1280×720, one slide per page</div>
        </div>
      </div>
    ...
    <div class="plm-export-card" style="border-color:#2d4a44;">
      <div class="plm-export-card-header">
        ...
        <div>
          ...
          <div class="plm-export-desc">All slides (every version), presentation doc, slide briefs,
            prompt history, cost summary, and source files — everything from this session.</div>
        </div>
      </div>
      <div style="display:flex;gap:8px;flex-wrap:wrap;">
        <button id="plm-export-archive-btn" class="plm-btn-primary plm-export-btn" style="background:#0d3d3a;border-color:#2c7a73;color:#4ECDC4;">Download Archive</button>
    ...
    <div id="plm-share-toast" style="display:none;background:#0d3d3a;border:1px solid #4ECDC4;border-radius:6px;padding:12px;font-size:12px;color:#a0aec0;line-height:1.6;"></div>
  `;
