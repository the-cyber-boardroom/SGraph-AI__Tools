 ...
 *
 * v0.1.3 changes:
 *  - Vault integration: _vault, _vaultDeckId, _vaultProfile, _vaultPendingChanges,
 *    _deckTitle state fields added.
 *  - SlideVersion typedef gains distBlob (Blob), liBlob (Blob), sizes ({png,dist,li}).
 *  - New methods: setVault(), vault, vaultDeckId, vaultConnected, vaultPendingChanges,
 *    setDeckTitle(), deckTitle, incrementPending(), clearPending(),
 *    vaultProfile getter/setter.
 *
 ...
 * @property {string}      imageSrc     - PNG data URL (in-memory display, full fidelity)
 * @property {Blob|null}   distBlob     - JPEG 1200px q82 (vault + PDF export, ~113 KB)
 * @property {Blob|null}   liBlob       - JPEG 800px q70 (social sharing, ~50 KB)
 * @property {{png:number, dist:number, li:number}|null} sizes - byte sizes per variant
...
export class PipelineState {
  constructor() {
    ...
    /** @type {{keys: object, apiBaseUrl: string, vaultId: string}|null} */
    this._vault = null;

    /** @type {string|null} Current deck's folder ID in vault (8-char hex) */
    this._vaultDeckId = null;

    /** @type {'full'|'distribution'|'minimal'} Vault storage profile */
    this._vaultProfile = 'full';

    /** @type {number} Count of local changes not yet committed to vault */
   ...
   * Stores distBlob and liBlob for vault upload and PDF export.
   *
   * @param {number} index
  ...
  /**
   * Store vault handle and deck ID. Resets pending change counter.
   *
   * @param {{keys: object, apiBaseUrl: string, vaultId: string}|null} vault
   * @param {string|null} deckId
   * @returns {void}
   */
  setVault(vault, deckId) {
    this._vault               = vault;
    this._vaultDeckId         = deckId;
    this._vaultPendingChanges = 0;
    this._notify();
  }

  /** @returns {{keys: object, apiBaseUrl: string, vaultId: string}|null} */
  get vault() { return this._vault; }

  /** @returns {string|null} */
  get vaultDeckId() { return this._vaultDeckId; }

  /** @returns {boolean} */
  get vaultConnected() { return this._vault !== null; }

  /** @returns {number} */
  get vaultPendingChanges() { return this._vaultPendingChanges; }

  /**
   * Increment the pending-changes counter by 1.
   *
   * @returns {void}
   */
  incrementPending() {
    this._vaultPendingChanges++;
    this._notify();
  }
  ...
  /** @returns {'full'|'distribution'|'minimal'} */
  get vaultProfile() { return this._vaultProfile; }

  /** @param {'full'|'distribution'|'minimal'} profile */
  set vaultProfile(profile) {
    this._vaultProfile = profile;
    this._notify();
  }

  /**
   * Set the deck display title.
   *
   * @param {string} title
   * @returns {void}
   */
  setDeckTitle(title) {
    this._deckTitle = title;
    this._notify();
  }

  /** @returns {string|null} */
  get deckTitle() { return this._deckTitle; }
  ...
  /**
   * Get the full pipeline state snapshot (JSON-serialisable; blobs omitted).
   *
   * @returns {object}
   */
  getFullState() {
    ...
    return {
      ...
      vaultConnected:  this.vaultConnected,
      vaultDeckId:     this._vaultDeckId,
      vaultProfile:    this._vaultProfile,
      vaultPending:    this._vaultPendingChanges,
      deckTitle:       this._deckTitle,
    };
  }
