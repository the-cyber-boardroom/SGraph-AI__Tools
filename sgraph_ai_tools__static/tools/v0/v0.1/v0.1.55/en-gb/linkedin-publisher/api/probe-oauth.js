...
const REDIRECT = 'https://dev.tools.sgraph.ai/en-gb/linkedin-publisher/oauth-callback.html';

/** @returns {Promise<{pass: boolean, detail: string}>} */
export async function runOauthAuthorizeProbe() {
    const scope = encodeURIComponent('openid profile email w_member_social');
    const url = `https://www.linkedin.com/oauth/v2/authorization?response_type=code&client_id=probe&redirect_uri=${encodeURIComponent(REDIRECT)}&scope=${scope}&state=phase0`;
    try {
        const res = await fetch(url, { method: 'GET', mode: 'cors' });
        return {
            pass: true,
            detail: `Reached server. HTTP ${res.status} — note: production flow uses popup navigation, not fetch, so this is diagnostic only.`,
        };
    } catch (err) {
        return {
            pass: false,
