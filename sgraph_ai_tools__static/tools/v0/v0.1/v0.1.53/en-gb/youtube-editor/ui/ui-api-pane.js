...
export function initApiPane(wrap) {
    ...
    devPanel.className = 'yte-dev-panel';
    ...
    tabBar.className = 'yte-dev-panel__tabs';
    devPanel.appendChild(tabBar);

    const content = document.createElement('div');
    content.className = 'yte-dev-panel__content';
    ...
    for (const t of DEV_TABS) {
        ...
        btn.type = 'button';
        btn.className = 'yte-dev-panel__tab' + (t.id === activeTab ? ' is-active' : '');
        ...
        pane.className = 'yte-dev-panel__pane';
    ...
    footer.className = 'yte-footer-bar';
    const inner = document.createElement('div');
    inner.className = 'yte-footer-bar__inner';
    inner.innerHTML = `
...
function _buildSkills() {
    ...
    wrap.className = 'yte-skills';
    for (const s of SKILLS) {
        const card = document.createElement('div');
        card.className = 'yte-skills__card';
        const hdr = document.createElement('div');
        hdr.className = 'yte-skills__header';
        hdr.innerHTML = `<span>${s.label}</span><span class="yte-skills__chev">▼ expand</span>`;
        const body = document.createElement('pre');
        body.className = 'yte-skills__body';
        body.textContent = 'Loading…';
        hdr.addEventListener('click', () => {
            const open = body.style.display === 'block';
            body.style.display = open ? 'none' : 'block';
            hdr.querySelector('.yte-skills__chev').textContent = open ? '▼ expand' : '▲ collapse';
