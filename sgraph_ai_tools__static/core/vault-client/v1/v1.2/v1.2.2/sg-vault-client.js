/* =============================================================================
   ...
   v1.2.2 — Browser-based vault access using Web Crypto API

   Changes from v1.2.1:
     - importReadKey(readKeyBase64Url): import a base64url-encoded 32-byte
       AES-256-GCM key directly. Used by the embed stack (sg-vault-key)
       to accept a pre-derived read_key without requiring the passphrase.

   All other exports unchanged from v1.2.1.
...
/**
 * Import a base64url-encoded 32-byte AES-256-GCM read key.
 * Used by the embed stack to accept a pre-derived read_key that has been
 * base64url-encoded and stored in a public manifest or page source.
 *
 * Mirrors the read_key import half of deriveVaultKeys() exactly.
 *
 * @param {string} readKeyBase64Url - Base64url-encoded 32-byte AES-256-GCM key
 * @returns {Promise<CryptoKey>} Imported AES-GCM CryptoKey for decrypt operations
 * @throws {Error} If the input is not a valid 32-byte base64url string
 */
export async function importReadKey(readKeyBase64Url) {
    const keyBytes = base64UrlToBytes(readKeyBase64Url)
