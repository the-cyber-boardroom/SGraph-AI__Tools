/* =============================================================================
   ...
   v1.0.1 — Two-branch model with push/pull, cache, and tree model

   v1.0.1 changes:
     - Re-pinned onto vault-cache v1.0.1 + vault-client v1.2.3, which together
       make the branch-index read in open() resolve to bare/indexes/ — where
       sgit writes it. Previously that GET went to bare/data/ and always 404'd,
       so a CLI vault's named HEAD was only ever found via the HMAC fallback
       and a CLI clone branch could not be seen at all.
     - No API change.
...
import { cachedFetch, cachedFetchJson, fetchRef, cachedFetchRaw }
    from '/core/vault-cache/v1/v1.0/v1.0.1/sg-vault-cache.js'
