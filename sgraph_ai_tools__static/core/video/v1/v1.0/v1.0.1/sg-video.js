 ...
 *
 * Changes from v1.0.0:
 *  - Added convertToMp4(webmBlob) — re-encodes a WebM blob to MP4 via FFmpeg WASM.
 *    If the input is already MP4 (e.g. from Safari MediaRecorder), it is returned unchanged.
 */
...
// ─────────────────────────────────────────────────────────────────────────────
// v1.0.1 addition: WebM → MP4 conversion
// ─────────────────────────────────────────────────────────────────────────────
...
export async function convertToMp4(webmBlob, onProgress) {
    ...
    await _writeInputFile(ffmpeg, inputName, webmBlob);
    ...
    const mp4Blob = await _readAndCleanup(ffmpeg, outputName, 'video/mp4');
    await ffmpeg.deleteFile(inputName);
