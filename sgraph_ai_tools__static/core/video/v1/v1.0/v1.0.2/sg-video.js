 ...
 *
 * Changes from v1.0.0:
 *  - convertToMp4(webmBlob) merged in from v1.0.1 — re-encodes a WebM blob to
 *    MP4 (libx264/aac/faststart); MP4/QuickTime inputs returned unchanged.
 * Changes from v1.0.1:
 *  - Restores v1.0.0's WORKERFS input mounting (bytes read on demand — the
 *    full file never has to fit the ~2 GB WASM heap), which v1.0.1 had
 *    replaced with a full writeFile copy. convertToMp4 uses WORKERFS too.
 */
...
// ─────────────────────────────────────────────────────────────────────────────
// Merged from v1.0.1: WebM → MP4 conversion (adapted to WORKERFS input)
// ─────────────────────────────────────────────────────────────────────────────

/**
 * Re-encode a WebM blob to MP4 using FFmpeg WASM.
 *
 * If the input blob is already MP4 (e.g. recorded on Safari via MediaRecorder),
 * it is returned unchanged without invoking FFmpeg — avoiding unnecessary work.
 *
 * Uses the x264 / AAC codec pair for maximum device compatibility.
 * Re-encoding is required because WebM containers are not natively playable
 * in all environments (e.g. iOS Safari, some Android players).
 *
 * @param {Blob} webmBlob  WebM blob from stopRecording(), or an MP4 blob (Safari)
 * @param {Function} [onProgress]  Called with { ratio } (0.0–1.0) during conversion
 * @returns {Promise<Blob>} MP4 blob (video/mp4)
 * @throws {Error} If FFmpeg fails or the input is not a video Blob
 */
export async function convertToMp4(webmBlob, onProgress) {
    if (!(webmBlob instanceof Blob)) {
        throw new Error('convertToMp4: expected a Blob');
    }

    // If already MP4 (Safari records MP4 natively), return as-is
    if (webmBlob.type.includes('mp4') || webmBlob.type.includes('quicktime')) {
        return webmBlob;
    }
    ...
    const inputPath  = await _mountInputFile(ffmpeg, webmBlob);
    ...
    try {
        ...
        if (exitCode !== 0) {
            throw new Error('convertToMp4: FFmpeg failed to convert the input.');
        }

        mp4Blob = await _readAndCleanup(ffmpeg, outputName, 'video/mp4');
    } finally {
        await _unmountInputFile(ffmpeg);
    }
