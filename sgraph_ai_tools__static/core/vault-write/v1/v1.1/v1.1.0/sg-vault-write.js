/* =============================================================================
   ...
   v1.1.0 — Sub-tree model with encrypted metadata
...
export async function writeVaultFile(vault, filePath, content, options = {}) {
    ...
    const uploadFn = async (objectId, encryptedBytes) => {
        await _putObject(apiBaseUrl, vaultId, keys.writeKey, objectId, encryptedBytes)
    }
    ...
    await _putObject(apiBaseUrl, vaultId, keys.writeKey, commitId, commitCipherBytes)
    ...
    await _putRef(apiBaseUrl, vaultId, keys.writeKey, keys.refFileId, refCipherBytes)
