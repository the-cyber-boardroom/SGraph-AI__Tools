...
/**
 * Normalise one entry from the `presigned/initiate` part list into
 * `{ url, partNumber }`.
 *
 * The API has been observed to return either a bare URL string per part
 * (`["https://…", …]`) or an object per part
 * (`[{ url: "https://…", part_number: 1 }, …]`). The old code assumed the
 * former and passed the object straight to `fetch()`, which stringified it to
 * `"[object Object]"` — a relative URL that resolved against the page origin and
 * failed with `501 Unsupported method ('PUT')` (the bug reported against the
 * video recorder's Upload & Share). This accepts both shapes and a range of key
 * spellings.
 *
 * @param {string|object} entry  Part descriptor from the initiate response.
 * @param {number}        index  Zero-based position in the returned list.
 * @returns {{ url: string|null, partNumber: number }}
 */
function normalisePart(entry, index) {
  ...
  if (entry && typeof entry === 'object') {
    ...
    return { url: typeof url === 'string' ? url : null, partNumber };
  }
  return { url: null, partNumber: index + 1 };
}
...
async function multipartUpload(ciphertext, transferId, sendUrl, accessToken, onProgress) {
  ...
  // Normalise every part descriptor to { url, partNumber } — the API may return
  // bare URL strings or per-part objects (see normalisePart).
  const partList = urls.map(normalisePart);
  const bad = partList.find(p => !p.url);
  if (bad) {
    throw new Error(
      `Multipart initiate returned a part without a usable URL — sample entry: ${JSON.stringify(urls[0])}`
    );
  }

  // Step 3 — upload parts in parallel (max MAX_CONCURRENT).
  // Derive each part's byte range from its (1-based) partNumber, not the array
  // index, so a server that returns parts out of order still maps bytes correctly.
  let completedParts = 0;
  const tasks = partList.map(({ url, partNumber }) => () => {
